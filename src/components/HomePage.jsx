                            ...
                            <div className={`card-surface p-4 rounded-lg transition-colors duration-300 ${theme === 'dark' ? 'bg-[#222]/80 border border-gray-700' : 'bg-white border border-gray-300'
                                ...
                                <div className="flex flex-col md:flex-row gap-4 items-start">
                                    ...
                                    <div>
                                        ...
                                        <p className={`mt-2 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>{featured.description}</p>
                                        <div className="mt-4 flex gap-2">
                                            <button onClick={() => onSelect(featured)} className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition">View</button>
                                            <button onClick={() => onNavigate('browse')} className={`px-3 py-2 border rounded-md transition ${theme === 'dark'
                                                    ? 'border-[rgba(255,255,255,0.04)] text-white hover:bg-gray-700'
                                ...
                                <div className={`card-surface p-4 rounded-lg transition-colors duration-300 ${theme === 'dark' ? 'bg-[#222]/80 border border-gray-700' : 'bg-white border border-gray-300'
                                    ...
                                    <div className="mt-3 grid grid-cols-2 gap-2">
                                        {movies.slice(0, 4).map((m) => (
                                            <button key={m.id} onClick={() => onSelect(m)} className={`text-left text-sm hover:text-red-600 transition ${theme === 'dark' ? 'text-gray-200 hover:text-red-400' : 'text-gray-800'
                                                }`}>{m.title}</button>
                                        ))}
                                    </div>
                                </div>
